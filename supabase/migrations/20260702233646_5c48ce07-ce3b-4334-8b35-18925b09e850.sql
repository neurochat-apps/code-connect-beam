...
-- Add category 00014 COMISIONES STRIPE to all existing workspaces
INSERT INTO public.categories (workspace_id, code, name, type, is_system)
SELECT w.id, '00014', 'COMISIONES STRIPE', 'egreso'::txn_type, true
FROM public.workspaces w
WHERE NOT EXISTS (
  SELECT 1 FROM public.categories c
  WHERE c.workspace_id = w.id AND c.code = '00014'
);

-- Update trigger to include 00014 for future workspaces
