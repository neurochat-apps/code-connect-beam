...
-- 1. Add neutro to txn_type
ALTER TYPE txn_type ADD VALUE IF NOT EXISTS 'neutro';

-- 2. Client enums
DO $$ BEGIN
  CREATE TYPE client_type AS ENUM ('recurrente','proyecto','cuota');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE client_status AS ENUM ('activo','pausado','completado');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 3. Client new columns
ALTER TABLE public.clients
  ADD COLUMN IF NOT EXISTS currency txn_currency NOT NULL DEFAULT 'COP',
  ADD COLUMN IF NOT EXISTS type client_type NOT NULL DEFAULT 'recurrente',
  ADD COLUMN IF NOT EXISTS monthly_amount numeric,
  ADD COLUMN IF NOT EXISTS project_total numeric,
  ADD COLUMN IF NOT EXISTS next_payment_date date,
  ADD COLUMN IF NOT EXISTS status client_status NOT NULL DEFAULT 'activo';

-- 4. Replace handle_new_user with new categories + clients
